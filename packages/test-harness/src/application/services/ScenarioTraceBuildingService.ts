...
export class ScenarioTraceBuildingService {
  ...
  private factorTrace(factor: string): BankActionEntity[] {
    const directTraces: Record<string, BankActionEntity[]> = {
      ...
      page_visibility: [
        this.action('page_hidden', 300),
        this.action('page_visible', 800, { reason: 'frequent_page_exits_during_payment_form' }),
      ],
      ...
      screen_sharing: [this.action('screen_sharing_observed', 300)],
